...
from src.black_scholes import d1, d2, call_price, put_price, call_delta, put_delta, price, delta, gamma, vega
from src.instrument import CallPut
...
def test_price(S, K, r, sigma, T, call_put, expected_price):
    prc = price(S, K, r, sigma, T, call_put)
...
def test_call_delta(S, K, r, sigma, T, expected_call_delta):
    _delta = call_delta(S, K, r, sigma, T) 
...
def test_put_delta(S, K, r, sigma, T, expected_put_delta):
    _delta = put_delta(S, K, r, sigma, T)
...
def test_delta(S, K, r, sigma, T, call_put, expected_delta):
    _delta = delta(S, K, r, sigma, T, call_put)
